...
ARG TEKU_BRANCH
ARG TEKU_REPO
RUN git clone --depth 1 --single-branch --branch "${TEKU_BRANCH}" "${TEKU_REPO}" && \
...
ARG BESU_BRANCH
ARG BESU_REPO
RUN git clone --depth 1 --single-branch --branch "${BESU_BRANCH}" "${BESU_REPO}" && \
    cd besu && \
...
ARG NETHERMIND_REPO
RUN git clone --depth 1 --single-branch --branch "${NETHERMIND_BRANCH}" "${NETHERMIND_REPO}" && \
    cd nethermind && \
...
ARG RETH_BRANCH
ARG RETH_REPO
RUN git clone --depth 1 --single-branch --branch "${RETH_BRANCH}" "${RETH_REPO}" && \
    cd reth && \
