...
make build-config config=builds/deneb-devnet-7/mainnet-testnet.yaml
docker tag "$CONFIG_IMAGE:latest" "$CONTAINER_REPOSITORY/$CONFIG_IMAGE:$TAG"

make build-config config=builds/deneb-devnet-7/mainnet-testnet-inst.yaml
...
docker tag "$ETB_ALL_CLIENTS:deneb-devnet-7" "$CONTAINER_REPOSITORY/$ETB_ALL_CLIENTS:$TAG"
docker tag "$ETB_ALL_CLIENTS_INST:deneb-devnet-7" "$CONTAINER_REPOSITORY/$ETB_ALL_CLIENTS_INST:$TAG"
