...
RUN make -j4 update
RUN make -j4 nimbus_beacon_node NIMFLAGS="-d:const_preset=minimal -d:web3_consensus_const_preset=minimal -d:disableMarchNative -d:FIELD_ELEMENTS_PER_BLOB=4 --cc:clang --clang.exe:clang-15 --clang.linkerexe:clang-15"
RUN mv /git/nimbus-eth2/build/nimbus_beacon_node /tmp/nimbus_beacon_node_uninstrumented
RUN make clean
RUN make -j4 USE_LIBBACKTRACE=0 nimbus_beacon_node NIMFLAGS="-d:const_preset=minimal -d:web3_consensus_const_preset=minimal -d:disableMarchNative -d:FIELD_ELEMENTS_PER_BLOB=4 --cc:clang --clang.exe:clang-15 --clang.linkerexe:clang-15 --passC:'-fno-lto -fsanitize-coverage=trace-pc-guard' --passL:'-fno-lto -L/usr/lib/ -lvoidstar'"
